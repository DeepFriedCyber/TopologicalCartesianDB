...
class PublicDatasetLoader:
    ...
    def load_dataset(self, dataset_name: str, sample_size: Optional[int] = None) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
        """
        ...
        try:
            with h5py.File(filepath, 'r') as f:
                ...
                if sample_size and len(train_vectors) > sample_size:
                    indices = np.random.choice(len(train_vectors), sample_size, replace=False)
                    train_vectors = train_vectors[indices]
                
                # Load test queries
                test_queries = f['test'][:]
                if sample_size and len(test_queries) > min(100, sample_size // 10):
                    query_size = min(100, sample_size // 10)
                    test_queries = test_queries[:query_size]
                
                # Load ground truth (nearest neighbors for evaluation)
                ground_truth = f['neighbors'][:len(test_queries)] if 'neighbors' in f else None
                
                logger.info(f"Loaded {len(train_vectors)} vectors, {len(test_queries)} queries")
    ...
    def batch_search(self, queries: np.ndarray, top_k: int) -> Tuple[List[Dict], float]:
        ...
        try:
            for query_vector in queries:
                ...
                hits = [
                    {
                        'id': hit.payload.get('vector_id', hit.id),
                        'score': hit.score
                    }
